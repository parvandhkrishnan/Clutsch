...
const Onboarding = () => {
  ...
  const dotStyle = (i) => ({
    ...
    height: '10px',
    borderRadius: '50%',
    background: step > i ? 'var(--color-accent)' : step === i ? 'var(--color-accent)' : 'var(--color-rule)',
  ...
  const welcomeStep = (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px', textAlign: 'center', padding: '40px 0', animation: 'fadeIn 250ms ease-out' }}>
      <div style={{ width: '80px', height: '80px', borderRadius: 'var(--radius-panel)', background: 'var(--color-accent-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  const integrationStep = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 250ms ease-out' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p className="text-body" style={{ color: 'var(--color-ink-2)' }}>Choose your primary communication channel to begin prioritization.</p>
      </div>
        ...
        {[
          ...
          return (
            <div key={item.id}
              ...
              style={{ 
                padding: '24px', 
                background: isConnected ? 'var(--color-accent-soft)' : 'var(--color-paper-2)',
                border: `1px solid ${isConnected ? 'var(--color-rule-strong)' : 'var(--color-rule)'}`,
  ...
  const surveyStep = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 250ms ease-out' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p className="text-body" style={{ color: 'var(--color-ink-2)' }}>Help us understand what matters most to you.</p>
      </div>
      <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '480px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <label className="text-label" style={{ color: 'var(--color-ink)', display: 'flex', alignItems: 'center', gap: '6px' }}><Target size={16} /> Key stakeholders</label>
          <input type="text" placeholder="e.g. CEO, Key Clients, Project Alpha Team" />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <label className="text-label" style={{ color: 'var(--color-ink)', display: 'flex', alignItems: 'center', gap: '6px' }}><Zap size={16} /> Primary focus this week</label>
          <input type="text" placeholder="e.g. Q3 Planning, Bug Triage, Client Onboarding" />
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <label className="text-label" style={{ color: 'var(--color-ink)', display: 'flex', alignItems: 'center', gap: '6px' }}><Users size={16} /> Top collaborators</label>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px', textAlign: 'center', padding: '40px 0', animation: 'fadeIn 250ms ease-out' }}>
      {syncStatus === 'syncing' ? (
        <>
          ...
          <p className="text-body" style={{ color: 'var(--color-ink-2)' }}>Our AI is analyzing your communications to build your first focus moment.</p>
        </>
      ) : (
        <>
          <div style={{ width: '80px', height: '80px', borderRadius: 'var(--radius-panel)', background: '#F0FDF4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'var(--color-paper)' }}>
